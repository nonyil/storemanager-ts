...
export class SalesByIdImplementation implements ISalesById {
  async GetAllSalesId(id: number): Promise<ISalesById2[]> {
    const query =
      await 'SELECT sale_id, date, product_id, quantity FROM sales AS SALES INNER JOIN sales_products AS SALES_PRODUCTS ON SALES_PRODUCTS.sale_id = SALES.id where id = ? ORDER BY sale_id ASC, product_id ASC';
